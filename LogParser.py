...
Parses ADIF log files and filters records based on LOTW_QSL_RCVD status
"""
...
class ADIFLogParser:
    ...
    def filter_records(self, filter_type: str = "confirmed", band_filter: str = "all") -> List[Dict[str, str]]:
        """Filter records based on LOTW_QSL_RCVD status and band"""
        filtered = []
        
        if filter_type == "confirmed":
            # Show only confirmed records (Y)
            for record in self.records:
                lotw_rcvd = record.get('LOTW_QSL_RCVD', 'N')
                if lotw_rcvd == 'Y':
                    filtered.append(record)
        elif filter_type == "unconfirmed":
            # Show only unconfirmed records (not Y or missing)
            for record in self.records:
                lotw_rcvd = record.get('LOTW_QSL_RCVD', 'N')
                if lotw_rcvd != 'Y':
                    filtered.append(record)
...
class ADIFLogGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Radiobutton(filter_frame, text="Confirmed records only (LOTW_QSL_RCVD = Y)", 
                       variable=self.filter_var, value="confirmed").grid(row=1, column=0, sticky=tk.W, pady=2)
        ttk.Radiobutton(filter_frame, text="Unconfirmed records only (LOTW_QSL_RCVD missing or ≠ Y)", 
                       variable=self.filter_var, value="unconfirmed").grid(row=2, column=0, sticky=tk.W, pady=2)
        ttk.Radiobutton(filter_frame, text="Unconfirmed records (within a Band for DXCC entities with no QSL-RCVD = Y)", 
                       variable=self.filter_var, value="unconfirmed_no_qsl").grid(row=3, column=0, sticky=tk.W, pady=2)
        ttk.Radiobutton(filter_frame, text="All records", 
                       variable=self.filter_var, value="all").grid(row=4, column=0, sticky=tk.W, pady=2)
        
        # Band filters
        ttk.Label(filter_frame, text="Band:", font=('TkDefaultFont', 9, 'bold')).grid(row=5, column=0, sticky=tk.W, pady=(15, 5))
        
        self.band_var = tk.StringVar(value="all")
        
        # Create band filter buttons frame
        band_frame = ttk.Frame(filter_frame)
        band_frame.grid(row=6, column=0, sticky=tk.W, pady=2)
        ...
        columns = ('CALL', 'BAND', 'DXCC', 'COUNTRY', 'MODE', 'FREQ', 'QSL-SENT', 'QSL-RCVD')
        ...
        self.tree.heading('QSL-SENT', text='QSL-SENT')
        ...
        self.tree.column('QSL-SENT', width=80)
    ...
    def apply_filter(self):
        ...
        # Sort records by band before displaying
        ...
        if file_path:
            try:
                with open(file_path, 'w') as f:
                    # Write header
                    f.write(f"{'CALL':<12} {'BAND':<8} {'DXCC':<6} {'COUNTRY':<20} {'MODE':<8} {'FREQ':<10} {'QSL-SENT':<10} {'QSL-RCVD':<10}\n")
                    f.write("-" * 96 + "\n")
                    
                    # Write records
                    for item in self.tree.get_children():
                        values = self.tree.item(item, 'values')
                        f.write(f"{values[0]:<12} {values[1]:<8} {values[2]:<6} {values[3]:<20} {values[4]:<8} {values[5]:<10} {values[6]:<10} {values[7]:<10}\n")
